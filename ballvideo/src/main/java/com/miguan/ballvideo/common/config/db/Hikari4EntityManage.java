...
@EnableJpaRepositories(
        ...
        basePackages = {"com.miguan.ballvideo.repositories4"}
)
public class Hikari4EntityManage {

    @Resource(name = "fourthDataSource")
    private HikariDataSource fourthDatasource;

    @Autowired
    private Environment environment;

    @Bean(name = "entityManager4",destroyMethod = "close")
    ...
    public LocalContainerEntityManagerFactoryBean entityManageFactory(EntityManagerFactoryBuilder builder){
        LocalContainerEntityManagerFactoryBean entityManagerFactory =  builder.dataSource(fourthDatasource)
                .packages("com.miguan.ballvideo.entity4").persistenceUnit("Hikari4EntityManage").build();
